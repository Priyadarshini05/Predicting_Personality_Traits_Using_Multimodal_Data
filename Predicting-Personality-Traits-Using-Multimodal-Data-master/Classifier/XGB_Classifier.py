...
def main():
    ...
    for traits in range(0,conf.n_traits):  
        ...
        # setup classifier
        ext_clf = xgb.XGBClassifier(gamma=conf.gamma,min_child_weight=conf.min_child_weight,max_delta_step=conf.max_delta_step,
        subsample=conf.subsample,colsample_bytree=conf.colsample_bytree,colsample_bylevel=conf.colsample_bylevel,
        reg_lambda=conf.reg_lambda,base_score=conf.base_score,objective='binary:logistic')
        ...
        with open('Classifier/XGB_train_history', 'a') as train_file:
        ...
        with open('Classifier/XGB_val_history', 'a') as val_file:
